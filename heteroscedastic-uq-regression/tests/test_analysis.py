...
MODULE_PATH = Path(__file__).resolve().parents[1] / "analysis" / "main_analysis.py"
spec = importlib.util.spec_from_file_location("main_analysis", MODULE_PATH)
...
def test_protocol_b_reuses_one_realized_dataset_within_outer_loop(tmp_path, monkeypatch) -> None:
    ...
    def fake_synthetic_dataset(seed: int):
        calls.append(seed)
        marker = float(seed)
        Xtr = np.full((8, 1), marker)
        ytr = np.arange(8, dtype=float)
        Xte = np.full((4, 1), marker)
        yte = np.arange(4, dtype=float)
    ...
    def fake_prepare(Xtr, ytr, Xte, yte):
        return Data(float(Xtr[0, 0]))
